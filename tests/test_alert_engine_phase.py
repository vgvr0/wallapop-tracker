...
def event(db, kind="price_drop", *, listing=10, search=None, profile=None, old="100", new="80"):
    ...
    with db.transaction() as s:
        ...
        run = TrackingRunRecord(
            profile_id=profile,
            tracked_search_id=None if profile else (search or 3),
            started_at=now,
            finished_at=now,
        ...
        listing_row = ListingRecord(
            id=listing,
            wallapop_item_id=str(listing),
            first_seen_at=now,
            last_seen_at=now,
            created_at=now,
            updated_at=now,
        ...
        s.flush()
        row = TrackingEventRecord(
            event_type=kind,
            idempotency_key=f"{kind}-{listing}-{search}-{profile}",
            listing_id=listing,
            tracking_run_id=run.id,
            tracked_search_id=search,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"search_id": 3}, 1),
        ({"profile_id": 7}, 1),
        ({"minimum_price_drop_percent": 10}, 1),
        ({"minimum_price_drop_percent": 30}, 0),
        ({"maximum_price": 90}, 1),
        ({"maximum_price": 70}, 0),
    ],
)
...
@pytest.mark.parametrize(
    "kind", ["tracking_run_failed", "tracking_run_degraded", "schema_drift_detected"]
)
...
async def test_webhook_statuses(status, retry):
    ...
    n = Notification(
        1, AlertType.LISTING_SOLD, "x", "title", None, None, Decimal("2"), datetime.now(UTC)
    )
...
async def test_telegram_failure_does_not_cancel_webhook(database):
    ...
    service = NotificationService(
        database,
        channels={
            "telegram": Channel(DeliveryResult(False, error="failed")),
    ...
    with database.session() as s:
        rows = {
            row.channel: row.status.value if hasattr(row.status, "value") else row.status
            for row in s.scalars(select(NotificationDeliveryRecord))
        }
