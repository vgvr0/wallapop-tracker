...
class AlertEngine:
    def __init__(self, database: Database, notification_service: NotificationService | None = None) -> None:
    ...
    def process_event(self, event_id: int) -> int:
        ...
        with self.database.transaction() as session:
            ...
            rules = list(session.scalars(select(AlertRuleRecord).where(
                AlertRuleRecord.enabled, AlertRuleRecord.event_type == event.event_type)))
            for rule in rules:
                if not self._matches(rule, event):
                    continue
                if rule.cooldown_seconds and session.scalar(select(NotificationDeliveryRecord).where(
                    NotificationDeliveryRecord.channel == rule.channel,
                    NotificationDeliveryRecord.status != "skipped",
                    NotificationDeliveryRecord.created_at >= now - timedelta(seconds=rule.cooldown_seconds),
                )):
                    continue
                ...
                if existing is None:
                    session.add(NotificationDeliveryRecord(event_id=event.id,
                        channel=rule.channel, destination=rule.destination, created_at=event.created_at,
    ...
    def _matches(rule: AlertRuleRecord, event: TrackingEventRecord) -> bool:
        ...
        if filters.get("maximum_price") is not None and (
            event.new_price is None
            or event.new_price > Decimal(str(filters["maximum_price"]))
        ):
            return False
